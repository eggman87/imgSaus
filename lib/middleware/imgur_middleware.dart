...
List<Middleware<AppState>> createImgurMiddleware([
  ...
  final loadItems = _createLoadGallery(repository);
  final filterItems = _createFilterGallery(repository);

  return [
    TypedMiddleware<AppState, LoadGalleryAction>(loadItems),
    TypedMiddleware<AppState, UpdateFilterAction>(filterItems)
  ];
}

Middleware<AppState> _createLoadGallery(GalleryRepository repository) {
  return (Store<AppState> store, action, NextDispatcher next) {
    LoadGalleryAction loadAction = action;

    repository.getItems(loadAction.section, loadAction.sort, loadAction.window, loadAction.page).then((items) {
      if (items.isOk()) {
